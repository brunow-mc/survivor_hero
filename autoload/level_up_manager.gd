...
func generate_upgrade_options(level: int) -> Array[Dictionary]:
	...
	for upgrade_data in attack_controller.attack_upgrades:
		...
		# Filtrar: não oferecer se já está no max_level
		...
		pool.append({
			...
			"icon": attack_controller.find_attack_data(upgrade_data.attack_id).icon if attack_controller.find_attack_data(upgrade_data.attack_id) else null  # v1.4.4
		})
	...
	for powerup_data in powerup_controller.powerups:
		...
		pool.append({
			...
			"icon": powerup_data.icon  # v1.4.4
		})
	
	print("   📦 Pool total: %d upgrades disponíveis" % pool.size())
	
	# Se pool vazio, retornar vazio
	if pool.size() == 0:
		push_warning("⚠️ LevelUpManager: Nenhum upgrade disponível!")
		return []
	
	# Sortear até 3 opções
...
func _apply_upgrade(player: Node, choice: Dictionary) -> void:
	...
	# Determinar nome do controller baseado no tipo
	...
	# Obter level ANTES de aplicar upgrade
	var level_before = _get_current_level(controller, choice)
	
	# Aplicar upgrade usando método unificado
	var success = controller.apply_upgrade(choice.id)
	
	# Log apropriado baseado no level REAL (não choice.current_level)
	if success:
