...
func check_and_teleport_distant_enemies(delta: float) -> void:
	...
	for enemy in enemies_to_teleport:
		...
		# Zera velocidade residual e força recálculo imediato de rota
		...
		if debug_enabled:
			...
			print("├─ Posição DEPOIS: (", int(enemy.global_position.x), ", ", int(enemy.global_position.y), ")")
...
func choose_enemy(available_budget: float = INF) -> EnemySpawnData:
	"""
	Escolhe qual inimigo spawnar baseado em tempo, budget e weight.
	"""
...
func group_adjacent_points(points: Array, threshold: float) -> Array:
	...
	var clusters: Array = []
	var visited: Array = []
	var adjacency_threshold: float = threshold * 1.5
...
func filter_offscreen_clusters(clusters: Array) -> Array:
	"""
	Filtra clusters mantendo APENAS pontos offscreen + margem de segurança.
	"""
...
func is_safe_from_other_enemies(check_pos: Vector2) -> bool:
	"""
	Verifica distância mínima de outros inimigos.
	"""
