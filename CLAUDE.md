...
- **The grid scan is the expensive part and is cached** (`get_navigable_grid()` — always call this, never `scan_navigable_grid()` directly). Each grid point costs one `NavigationServer2D` query plus one physics query, and halving `grid_sample_spacing` quadruples the point count. But navmesh and walls are **static**: the result only changes when the *camera* moves, so it is reused until the camera travels more than half a cell (or the spacing changes). The cheap offscreen filter still runs fresh on every call, so a slightly stale band degrades safely — it can only shift the candidate set, never approve an invalid point.
