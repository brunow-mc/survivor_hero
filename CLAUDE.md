...
**Enemies crossing a closed Barrier turn semi-transparent** (`EnemyBase.set_phasing()`, export `phase_alpha`) via `EnemyPhaseSensor`, polled every `_physics_process` (not signal-based — `body_entered`/`body_exited` proved unreliable against abrupt knockback displacement) and **reapplied every frame** while overlapping, not just on entry, so the effect self-heals if anything else touches `modulate` mid-overlap (see the `modulate` split note in *Enemy system*).
