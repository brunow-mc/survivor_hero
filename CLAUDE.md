...
3. **Being a child of the Y-sort container** — Y-sort only sorts CHILDREN of a `y_sort_enabled` parent. Each stage instances **`entities/scene/y_sort_organization.tscn`** (a configured, permanently-empty Node2D: z 2, relative, y-sort on). **The membership criterion is functional and single: anything that needs variable occlusion along the Y axis (Godot's Y-sort) goes inside the container; anything that doesn't stays outside.**
   ...
   Content placed in the editor inside the instance belongs to the stage file; never add children to the base scene `y_sort_organization.tscn` (they would be shared across every stage).
